...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        for(unsigned int i=0; i<NUM_ELEMS/4*4; i+=4){
            ...
            __m128i mid = _mm_cmpgt_epi32(tmp, _127);
            tmp = _mm_and_si128(tmp, mid);
        ...
        for(unsigned int w = NUM_ELEMS/4*4; w < NUM_ELEMS; w++) {
            if(vals[w]>127){
                result += vals[w];
        }
    }
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        _mm_storeu_si128((__m128i *) sum_arr, sum_vec);
        result = result + sum_arr[0] + sum_arr[1] + sum_arr[2] + sum_arr[3];
        for(unsigned int w = NUM_ELEMS/16*16; w < NUM_ELEMS; w++) {
            if (vals[w]>127){
                result += vals[w];
            }
        }
