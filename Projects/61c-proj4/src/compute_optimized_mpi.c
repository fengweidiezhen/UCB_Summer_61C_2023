...
int32_t dot(uint32_t n, int32_t *vec1, int32_t *vec2) {
  ...
  if (n >= 8) {
    __m256i const reverse = _mm256_setr_epi32(7,6,5,4,3,2,1,0);

    __m256i sum = _mm256_setzero_si256();

    for (uint32_t i = 0; i < n/8*8; i+=8) {

      __m256i b_value = _mm256_loadu_si256((__m256i *) (vec2 + (n - (i + 8))));
      __m256i a_value = _mm256_loadu_si256((__m256i *) (vec1 + i));
      
      __m256i b_reverse = _mm256_permutevar8x32_epi32(b_value, reverse);
      __m256i tem = _mm256_mullo_epi32(a_value, b_reverse);
...
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  ...
  int a_col = a_matrix->cols;
  int a_row = a_matrix->rows;

  int b_col = b_matrix->cols;
  int b_row = b_matrix->rows;

  int output_row = a_row - b_row + 1;
  int output_col = a_col - b_col + 1;
